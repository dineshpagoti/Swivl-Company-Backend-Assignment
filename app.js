...
app.post('/diary', authenticateToken, (req, res) => {
  ...
  db.run(
    `INSERT INTO diary_entries (title, description, date, location, photos, user_id) VALUES (?, ?, ?, ?, ?, ?)`,
    [title, description, date, location, photos, userId],
    function (err) {
      if (err) {
        return res.status(500).json({error: err.message})
      }
...
app.put('/diary/:id', authenticateToken, (req, res) => {
  ...
  const userId = req.user.id
  db.run(
    `UPDATE diary_entries SET title = ?, description = ?, date = ?, location = ?, photos = ? WHERE id = ? AND user_id = ?`,
    [title, description, date, location, photos, id, userId],
    function (err) {
      if (err) {
        return res.status(500).json({error: err.message})
      }
